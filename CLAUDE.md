...
**Data files:**
- `bank.js` / `bank2.js` / `bank3.js` / `bank4.js` — built-in sentence bank (140+210+180+220 = 750 sentences, loaded in that order via `<script>` tags and each `.concat()`-ing onto `window.BANK`), array of `{id, en, zh, cat, sc, lv, note}`. `id` prefix is `d`/`t`/`v` for daily/travel/vocab; keep new ids past the current max per prefix to avoid collisions — check with e.g. `grep -oE 'id:"[a-z][0-9]+"' bank*.js`. Adding sentences by hand means editing these files directly (there's no data-entry UI for the *built-in* bank — user/AI-added sentences instead go into `S.custom`, which needs no file edit or redeploy). Any new bank file must also be added to `sw.js`'s `ASSETS` array and to `使用說明.md`/`開始使用.md`'s file-upload lists.
